"""Unit tests for the comp2030.cipher_string module."""

import unittest

import comp2030.cipher_string
from comp2030.cipher_string import CipherString
...
class CipherStringTest(unittest.TestCase):
    ...
    def test_from_string_valid(self):
        # Empty string is OK
        self.assertTrue(comp2030.cipher_string.is_valid(''))
        cs1 = CipherString.from_string('')
        self.assertEqual('', str(cs1))

        self.assertTrue(comp2030.cipher_string.is_valid('ABC123'))
        cs2 = CipherString.from_string('ABC123')
        self.assertEqual('ABC123', str(cs2))

    def test_from_string_invalid(self):
        # Contains lower case letters
        self.assertFalse(comp2030.cipher_string.is_valid('abc123'))
        with self.assertRaises(ValueError):
            CipherString.from_string('abc123')

        # Contains a space
        self.assertFalse(comp2030.cipher_string.is_valid('ABC 123'))
        with self.assertRaises(ValueError):
            CipherString.from_string('ABC 123')

        # Contains Arabic-Indic digits
        self.assertFalse(comp2030.cipher_string.is_valid('ABC١٢٣'))
        with self.assertRaises(ValueError):
